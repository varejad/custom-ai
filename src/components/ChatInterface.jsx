...
export default function ChatInterface() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      addMessage(activeChat.id, { id: crypto.randomUUID(), role: 'assistant', content: `**${t('error')}**: ${error.message}. ${t('checkApiKey')}` });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', position: 'relative' }}>
      ...
      <div style={{ backgroundColor: 'var(--bg-secondary)', padding: '1rem', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.875rem', color: 'var(--text-muted)' }}>
        ...
        <button className="btn-icon" onClick={() => setShowSettings(true)} title={t('editChatBtnTitle')} style={{ padding: '0.25rem' }}>
          <Settings2 size={18} />
        </button>
      </div>
        ...
        <div style={{ maxWidth: '800px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          {activeChat.messages.length === 0 && (
            <div style={{ textAlign: 'center', marginTop: '4rem' }}>
              ...
              <h3 style={{ color: 'var(--text-primary)', marginBottom: '0.5rem' }}>{t('readyToChat')}</h3>
          ...
          {isLoading && (
            <div style={{ display: 'flex', justifyContent: 'flex-start' }}>
              <div style={{ padding: '1rem 1.25rem', backgroundColor: 'var(--bg-secondary)', borderRadius: '1rem', borderBottomLeftRadius: '0' }}>
                <span className="pulsing-dots">{t('thinking')}</span>
              </div>
            </div>
          )}
